...
import { JFKScale } from './components/JFKScale';
...
const DashboardScreen: React.FC = () => {
    ...
    const { categories, patients } = useContext(PatientsContext)!;
    
    // Fetch summary data from context if possible, otherwise rely on tasks.
    // For now we assume TasksProvider has summary or we compute it.
    const { summary } = useContext(TasksContext) as any;
    ...
    // 2. Gráfico de Barras (Pendências)
    const alertChartData = useMemo(() => {
        const counts = tasks.filter(t => t.status === 'alerta' || t.status === 'fora_do_prazo').reduce((acc, task) => {
            ...
            <div>
                ...
                <div className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm">
                    {alertChartData.length > 0 ? (
                        ...
                        <p className="text-center text-slate-500 dark:text-slate-400 py-4">Nenhuma pendência no momento.</p>
                    )}
                </div>
            </div>
        </div>
...
const PatientListScreen: React.FC = () => {
   ...
   // ✅ Como deve ficar (Copia e cola):
const filteredPatients = useMemo(() => {
    // 1. Primeiro filtra (busca por nome/leito)
    const filtered = patients.filter(p => 
        p.name.toLowerCase().includes(searchTerm.toLowerCase()) || 
        p.bedNumber.toString().includes(searchTerm)
    );

    // 2. Depois ordena do menor para o maior
    return filtered.sort((a, b) => {
        // Transformamos em número para garantir a ordem matemática
        return Number(a.bedNumber) - Number(b.bedNumber);
    });
}, [patients, searchTerm]);
...
const PatientHistoryScreen: React.FC = () => {
    ...
    const patientHistory = useMemo(() => {
        ...
        patient.cultures?.forEach(culture => {
            events.push({
                timestamp: new Date(culture.date).toISOString(),
                icon: BugIcon,
                description: `Cultura Coletada: ${culture.microorganism} - Local: ${culture.location}`,
                hasTime: false,
            });
        });
    ...
    const handleGeneratePdf = () => {
        ...
        const generateCulturesList = () => patient.cultures?.map(c => `
            <li>
                <strong>${c.microorganism}</strong> - Local: ${c.location}<br>
                Data: ${formatDate(c.date)}
            </li>
        `).join('') || '<li>Nenhuma cultura registrada.</li>';
...
const PatientDetailScreen: React.FC = () => {
    ...
    const handleSaveScaleScore = (data: { scaleName: string; score: number; interpretation: string; }) => {
     ...
     const handleDeleteDevice = (patientId: number | string, deviceId: number | string) => {
        if(window.confirm("Tem certeza que deseja arquivar este dispositivo? Ele sairá da lista visual.")) {
            deleteDeviceFromPatient(patientId, deviceId);
            showNotification({ message: 'Dispositivo arquivado com sucesso.', type: 'info' });
        }
    };

    const handleDeleteExam = (patientId: number | string, examId: number | string) => {
         if(window.confirm("Tem certeza que deseja arquivar este exame? Ele sairá da lista visual.")) {
    ...
    const dataTabs = [
        ...
        { id: 'cultures', label: 'Culturas', icon: BugIcon },
    ];
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm">
                ...
                <div className="border-b border-slate-200 dark:border-slate-800">
                    <nav className="flex justify-around">
                        <button
                            ...
                            className={`flex-1 py-4 px-1 text-center font-bold flex items-center justify-center gap-2 transition-colors duration-200 text-lg ${
                                mainTab === 'data'
                ...
                {mainTab === 'data' && (
                    <>
                        <div className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 overflow-x-auto">
                             <nav className="-mb-px flex justify-around min-w-max">
                                {dataTabs.map(tab => (
                                    <button
                                        key={tab.id}
                                        onClick={() => setDataTab(tab.id as any)}
                                        className={`flex-1 py-3 px-3 text-center font-semibold flex items-center justify-center gap-2 transition-colors duration-200 text-sm whitespace-nowrap ${
                                            dataTab === tab.id
                                ...
                                <>
                                    {patient.cultures?.map(culture => (
                                        <div key={culture.id} className="bg-slate-50 dark:bg-slate-800 p-3 rounded-lg">
                                            <div className="flex justify-between items-start">
                                                <div className="flex items-start gap-3">
                                                    <BugIcon className="w-5 h-5 text-blue-600 dark:text-blue-400 mt-1 flex-shrink-0" />
                                                    <div>
                                                        <p className="font-bold text-slate-800 dark:text-slate-200">{culture.microorganism}</p>
                                                        <p className="text-sm text-slate-500 dark:text-slate-400">Local: {culture.location}</p>
                                                        <p className="text-sm text-slate-500 dark:text-slate-400">Data: {new Date(culture.date).toLocaleDateString('pt-BR')}</p>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-1 flex-shrink-0 ml-2">
                    ...
                    <div className="p-4">
                        {scaleView === 'list' && (
                            <div className="space-y-3">
                                ...
                                <div onClick={() => setScaleView('jfk')} className="bg-slate-50 dark:bg-slate-800 p-3 rounded-lg flex justify-between items-center cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 transition">
                                    <div className="flex items-center gap-3"><BrainIcon className="w-5 h-5 text-blue-600 dark:text-blue-400" /><div><p className="font-bold text-slate-800 dark:text-slate-200">JFK Emergence Scale</p></div></div><ChevronRightIcon className="w-5 h-5 text-slate-400" />
                                </div>
                        ...
                        {scaleView === 'jfk' && (<div className='bg-slate-800 rounded-xl overflow-hidden -m-4'><button onClick={() => setScaleView('list')} className="flex items-center gap-2 text-sm text-blue-500 dark:text-blue-400 font-semibold mb-4 p-4 hover:bg-slate-700 w-full text-left"><BackArrowIcon className="w-4 h-4" />Voltar para Escalas</button><div className="p-4 pt-0"><JFKScale onSaveScore={handleSaveScaleScore} /></div></div>)}
                    </div>
                )}
            </div>
...
const AddCultureModal: React.FC<{ patientId: number | string; onClose: () => void;}> = ({ patientId, onClose }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if(!location || !microorganism || !date) return;
        addCultureToPatient(patientId, { location, microorganism, date });
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-30">
            <div className="bg-white dark:bg-slate-900 p-6 rounded-lg shadow-xl w-full max-w-sm m-4">
                 <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200">Cadastrar Cultura</h2>
                    <button onClick={onClose}><CloseIcon className="w-6 h-6 text-slate-500 dark:text-slate-400" /></button>
                </div>
                <form onSubmit={handleSubmit} className="space-y-4">
                     <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Local</label>
                        <input type="text" placeholder="Ex: Hemocultura" value={location} onChange={e => setLocation(e.target.value)} className="mt-1 block w-full border bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-700 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-slate-800 dark:text-slate-200" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Microorganismo</label>
                        <input type="text" placeholder="Ex: Staphylococcus aureus" value={microorganism} onChange={e => setMicroorganism(e.target.value)} className="mt-1 block w-full border bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-700 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-slate-800 dark:text-slate-200" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Data</label>
                        <input type="date" value={date} onChange={e => setDate(e.target.value)} className="mt-1 block w-full border border-slate-300 dark:border-slate-700 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200" />
            ...
            {filteredTasks.map((task: Task) => {
                ...
                return (
                    <div key={task.id} className={`bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border-l-4 border-${config.color}-500`}>
                        <div className="flex justify-between items-start">
                             <div>
                                <p className="font-bold text-slate-800 dark:text-slate-200 whitespace-pre-wrap">{task.description}</p>
...
const SettingsScreen: React.FC = () => {
    ...
    const handleSave = () => {
        updateUser({ name, title, sector, avatarUrl: avatarPreview });
